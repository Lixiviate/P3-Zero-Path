...
const SlidePanel = ({ isOpen, onClose, children }) => {
  return (
    <>
      {isOpen && (
        <div
          className="fixed inset-0 bg-zinc-950 bg-opacity-75 transition-opacity z-40"
          onClick={onClose}
        />
      )}

      <div
        className={`fixed right-0 top-16 h-[calc(100vh-4rem)] w-96 bg-white shadow-2xl transform transition-transform duration-300 ease-in-out z-50 ${
...
const CarbonCalculator = () => {
  ...
  const validateInputs = useCallback(() => {
    if (calculationType === CALCULATION_TYPES.FLIGHT) {
      if (
        !formData.departure?.match(/^[A-Z]{3}$/) ||
        !formData.destination?.match(/^[A-Z]{3}$/)
      ) {
  ...
  const handleInputChange = (e) => {
    ...
    setFormData((prev) => ({
      ...prev,
      [name]:
        name === "passengers" || name === "distance" || name === "weight"
          ? parseFloat(value)
          : value,
    }));
    setError(null);
          ...
          <div key={index} className="p-6 hover:bg-gray-50">
            <div className="flex justify-between items-center mb-3">
              ...
              <span className="text-sm text-blue-600 bg-blue-50 px-2 py-1 rounded-full">
                {item.result.carbon_kg ||
                  Math.round(item.result.carbon_g / 1000)}{" "}
                kg CO2
              </span>
            </div>
            ...
            <div className="text-sm text-gray-600">
              {item.type === CALCULATION_TYPES.FLIGHT && (
                <>
                  <p>
                    From: {item.details.departure} To:{" "}
  ...
  const renderInputFields = () => {
    const commonInputClasses =
      "w-full p-3 text-lg rounded-lg border border-gray-200 focus:ring-2 focus:ring-blue-500 focus:border-transparent";
          ...
          <div className="space-y-4">
            <div>
              ...
              >
                ...
                {Object.entries(VEHICLE_EMISSION_FACTORS).map(
                  ([type, factor]) => (
      ...
      <div className="bg-white rounded-xl shadow-lg overflow-hidden">
            ...
            >
              {Object.entries(CALCULATION_TYPES).map(([key, value]) => (
                <option key={key} value={value}>
                  Calculate {key.charAt(0) + key.slice(1).toLowerCase()}{" "}
                  Emissions
                </option>
              ))}
            </select>
        ...
        <div className="p-8 space-y-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4">{renderInputFields()}</div>
