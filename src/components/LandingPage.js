...
const LandingPage = () => {
  ...
  const createRipple = useCallback((x, y) => {
    ripplesRef.current.push({ x, y, radius: 0, maxRadius: Math.random() * 100 + 100, speed: Math.random() * 1 + 0.5, opacity: 1 });
  }, []);
  ...
  const animate = useCallback(() => {
    ...
    currents.forEach(current => {
      ...
      for (let x = 0; x < canvas.width; x += 10) {
        const y = current.y + Math.sin(x / current.wavelength) * current.amplitude;
        ctx.lineTo(x, y);
      }
      ctx.stroke();
    });
      ...
      <div className="relative z-10 text-center">
        ...
        <div className="bg-blue-400 bg-opacity-40 backdrop-filter backdrop-blur-lg rounded-xl p-8 shadow-xl max-w-md w-full">
          ...
          <form className="space-y-4">
            <div>
              <label htmlFor="username" className="block text-sm font-medium text-white mb-1 text-left">Username</label>
              <input 
                type="text" 
                id="username" 
                name="username" 
            ...
            <div>
              <label htmlFor="password" className="block text-sm font-medium text-white mb-1 text-left">Password</label>
              <input 
                type="password" 
                id="password" 
                name="password" 
